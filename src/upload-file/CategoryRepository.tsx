import { addDoc, collection, deleteDoc, doc, getDocs, runTransaction, updateDoc, writeBatch } from 'firebase/firestore';
...
export class CategoryRepository extends BaseRepository<Category> {
  ...
  async moveStudyMaterial(categorysorc:Category,categorydest:Category,docID:string,data:StudyMaterial){
    ...
    const studyMaterialsorcRef=collection(indexsorcRef,this.supCollection).withConverter(createStudyMaterialConverter(categorysorc.category));
    batch.delete(doc(studyMaterialsorcRef, docID));
    const indexdestRef =doc(this._collection,categorydest.id);
    const studyMaterialdestRef=collection(indexdestRef,this.supCollection).withConverter(createStudyMaterialConverter(categorydest.category));
